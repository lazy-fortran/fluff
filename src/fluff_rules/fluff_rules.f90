module fluff_rules
    ...
    use fluff_core
    use fluff_ast, only: fluff_ast_context_t, NODE_DECLARATION, &
                         NODE_IDENTIFIER, &
    ...
    use fortfront, only: symbol_info_t, variable_usage_info_t, &
                         get_variables_in_expression, &
                         get_identifiers_in_subtree, identifier_node, &
                         declaration_node, &
...
contains
    ...
    function get_style_rules() result(rules)
        ...
        rules(3)%description = &
            "Line exceeds maximum length with suggested breaking points"
            ...
            if (should_check_indent(node_type)) then
                if (mod(actual_indent, 4) /= 0) then  ! Check for 4-space indentation
                   ...
                   message="Inconsistent indentation (expected multiple of 4 spaces)", &
                        ...
                        call add_indentation_fix(violations(violation_count), &
                                                 location, actual_indent)
                    end if
                end if
            end if
        end if
    ...
    subroutine analyze_line_lengths_from_text(source_text, violations, &
    ...
    subroutine analyze_trailing_whitespace_from_text(source_text, violations, &
        ...
        do while (pos <= len(source_text))
            ...
            if (next_pos == 0) then
                ...
                if (line_end >= line_start .and. source_text(line_end:line_end) == &
                    char(13)) then
                    line_end = line_end - 1
                end if
                pos = pos + next_pos
    ...
    ! Find the actual end of non-whitespace content (excluding trailing spaces AND tabs)
                    ...
                    do i = line_length, 1, -1
                        if (line_content(i:i) /= ' ' .and. line_content(i:i) /= &
                            char(9)) then
                ...
                if (.not. is_comment_line(line_content)) then
                    ...
                    do i = 1, len(line_content)
                        ...
                        if (.not. in_string) then
                            if (line_content(i:i) == '"' .or. line_content(i:i) &
                                == "'") then
                                in_string = .true.
                            ...
                            else if (line_content(i:i) == '!' .and. .not. &
                                     in_string) then
                                ! Stop at comment
                                exit
                            end if
            ...
            if (line_end >= line_start) then
                ...
                if (.not. is_comment_line(line_content)) then
                    ...
                    if (common_pos > 0) then
                        if (is_keyword_at_position(line_lower, 'common', &
                                                   common_pos)) then
                            violation_count = violation_count + 1
                            if (violation_count <= size(violations)) then
                                location%start%line = line_num
                    ...
                    if (equiv_pos > 0) then
                        if (is_keyword_at_position(line_lower, 'equivalence', &
                                                   equiv_pos)) then
                            violation_count = violation_count + 1
                            if (violation_count <= size(violations)) then
                                location%start%line = line_num
    ...
    subroutine analyze_redundant_continue_from_text(source_text, violations, &
                                                    violation_count)
        character(len=*), intent(in) :: source_text
        type(diagnostic_t), intent(inout) :: violations(:)
        integer, intent(inout) :: violation_count
            ...
            if (line_end >= line_start) then
                ...
                if (.not. is_comment_line(line_content)) then
                    ...
                    if (continue_pos > 0) then
                        ...
                        if (is_keyword_at_position(line_lower, 'continue', &
                                                   continue_pos)) then
        ...
        do while (pos <= len(source_text))
            ...
            if (line_end >= line_start) then
                ...
                if (.not. is_comment_line(line_content)) then
                    ...
                    else if (index(line_lower, 'module ') == 1 .and. index(line_lower, &
                                                          'module procedure') /= 1) then
                        stack_depth = stack_depth + 1
                        name_start = 8
                        name_end = len(line_trimmed)
                        stack_names(stack_depth) = &
                    ...
                    do while (i <= len(line_trimmed) - 2)
                     ...
                     ! Check context - not a function call and not needed for precedence
                           ...
                           ! Check if it's in an if statement or other control structure
                                        if (.not. &
                                            is_in_control_structure(line_trimmed, &
                                                                    i)) then
    ...
    function needs_precedence_parentheses(line, start_pos, end_pos) &
    ...
    subroutine analyze_unused_variables_from_text(source_text, violations, &
                                                  violation_count)
        character(len=*), intent(in) :: source_text
        type(diagnostic_t), intent(inout) :: violations(:)
        integer, intent(inout) :: violation_count
            ...
            if (line_end >= line_start) then
                ...
                if (.not. is_comment_line(line_content)) then
                    ...
                    if (index(line_trimmed, '::') > 0) then
                        if (has_type_keyword(line_lower(1:index(line_trimmed, &
                                                                '::')))) then
                            call &
                               extract_variable_names(line_trimmed(index(line_trimmed, &
                                                                          '::') + 2:), &
...
! Check for variable usage (simplified - looks for identifiers in assignments and calls)
                    if (in_procedure) then
                        call extract_used_variables(line_trimmed, used_vars, &
                                                    used_count, declared_vars, &
                                                        declared_count)
                    end if
                end if
            end if
    ...
    subroutine extract_used_variables(line, used_vars, used_count, declared_vars, &
                                      declared_count)
        character(len=*), intent(in) :: line
    ...
    subroutine check_indentation_consistency(indent_levels, line_count, violations, &
                                             violation_count)
    ...
    pure integer function enclosing_unit_index(arena, node_index) result(unit_index)
        use fortfront, only: ast_arena_t, program_node, function_def_node, &
                             subroutine_def_node
        type(ast_arena_t), intent(in) :: arena
        integer, intent(in) :: node_index
        integer :: current
    ...
    subroutine check_f006_unused_variable(ctx, node_index, violations)
        ...
        do i = 1, size(declared)
            if (violation_count >= size(violations)) exit
    ...
    subroutine check_f007_undefined_variable(ctx, node_index, violations)
        ...
        do i = 1, ctx%arena%size
            ...
            if (violation_count > size(all_violations)) then
                call grow_violation_array(all_violations, size(all_violations)*2)
            end if
            ...
            all_violations(violation_count) = create_diagnostic( &
                                              code="F007", &
                                message="Undefined variable: "//trim(identifier_name), &
                                              file_path=current_filename, &
                                              location=location, &
    ...
    recursive subroutine check_intent_recursive(ctx, node_index, violations, &
                                                violation_count)
        type(fluff_ast_context_t), intent(in) :: ctx
        integer, intent(in) :: node_index
        type(diagnostic_t), intent(inout) :: violations(:)
        ...
        if (node_type == NODE_FUNCTION_DEF .or. node_type == NODE_SUBROUTINE_DEF) then
            ...
            call check_procedure_arguments_intent(ctx, node_index, violations, &
                                                  violation_count)
        end if
        ...
        do i = 1, num_children
            if (children(i) > 0) then
                call check_intent_recursive(ctx, children(i), violations, &
                                            violation_count)
            end if
        end do
    ...
    subroutine check_procedure_arguments_intent(ctx, proc_node, violations, &
                                                violation_count)
        type(fluff_ast_context_t), intent(in) :: ctx
    ...
    subroutine check_f015_redundant_continue(ctx, node_index, violations)
        ...
        call analyze_redundant_continue_from_text(current_source_text, &
                                                  temp_violations, violation_count)
    ...
    recursive subroutine analyze_nested_loops(ctx, node_index, violations, &
                                              violation_count)
        type(fluff_ast_context_t), intent(in) :: ctx
        integer, intent(in) :: node_index
        type(diagnostic_t), intent(inout) :: violations(:)
        ...
        if (node_type == NODE_DO_LOOP) then
            ...
            if (nested_loop_count > 1) then
                ...
                ! In practice, analyze array indexing patterns.
                if (violation_count < size(violations)) then
                    violation_count = violation_count + 1
                    violations(violation_count) = create_diagnostic( &
                                                  code="P002", &
                        message="Consider loop ordering for memory efficiency " // &
    ...
    ! Analyze procedures for pure/elemental opportunities
    recursive subroutine analyze_procedure_purity(ctx, node_index, violations, &
                                                  violation_count)
        type(fluff_ast_context_t), intent(in) :: ctx
        integer, intent(in) :: node_index
        type(diagnostic_t), intent(inout) :: violations(:)
        ...
        if (node_type == NODE_FUNCTION_DEF .or. node_type == NODE_SUBROUTINE_DEF) then
            ...
            if (.false.) then
                if (violation_count < size(violations)) then
                    ...
                    call add_pure_attribute_fix(violations(violation_count), &
        ...
        do i = 1, size(children)
            if (children(i) > 0) then
                call analyze_procedure_purity(ctx, children(i), violations, &
                                              violation_count)
            end if
        end do
    ...
    subroutine add_indentation_fix(diagnostic, location, actual_indent)
        ...
        ! Create fix suggestion
        fix%description = &
            "Fix indentation to "//trim(adjustl(char(correct_indent/4)))//" levels"
        fix%is_safe = .true.
    ...
    function check_implicit_none_in_current_source(ctx) result(found)
        ...
        ! Check if implicit none appears anywhere - for standalone subroutines/functions
            ...
            found = index(source_lower, "implicit") > 0 .and. index(source_lower, &
                                                                    "none") > 0
        end if
